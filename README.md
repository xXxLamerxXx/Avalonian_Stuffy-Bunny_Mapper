# 🐰 Avalonian Bunny-Mapper

**Интерактивный помощник для исследования порталов Авалона в Albion Online.**

Нажмите горячую клавишу, выделите название карты — и получите полный список сундуков, подземелий и ресурсов с размерами и количеством. Всё отображается в стильном полупрозрачном оверлее поверх игры.
...
1. Скачайте последнюю версию из [релизов](https://github.com/xXxLamerxXx/Avalonian_Stuffy-Bunny_Mapper/releases) (архив `AvalonMapper.zip` или `AvalonMapper.7z`).
2. Распакуйте архив в любое удобное место.
3. Запустите `.exe` — откроется окно настроек.
4. В игре нажмите назначенную горячую клавишу (по умолчанию `F9`), затем левой кнопкой мыши выделите название портала.
5. Для выхода из режима выделения нажмите `Esc`.

> **Важно:** не перемещайте `.exe` файл отдельно от остальных файлов — программа ожидает, что `maps.json`, папка `icons` и `Tesseract` находятся рядом с исполняемым файлом.
