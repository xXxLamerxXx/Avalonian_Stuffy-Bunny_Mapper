...
1. Скачайте последнюю версию из [релизов](https://github.com/xXxLamerxXx/Avalonian_Stuffy-Bunny_Mapper/releases) (архив `.zip` или `.7z`).
2. Распакуйте архив в любое удобное место.
3. Запустите `.exe` — откроется окно настроек.
4. В игре нажмите назначенную горячую клавишу (по умолчанию `F9` или `-`), затем левой кнопкой мыши выделите название портала.
5. Для выхода из режима выделения нажмите `Esc`.

> **Важно:** не перемещайте `.exe` файл отдельно от остальных файлов — программа ожидает, что `maps.json`, папка `icons` и `Tesseract` и `_internal` находятся рядом с исполняемым файлом.
