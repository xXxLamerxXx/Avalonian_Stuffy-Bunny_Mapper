...
1. Скачайте последнюю версию из [релизов](https://github.com/твой_логин/avalon-mapper/releases) (архив `AvalonMapper.zip`).
2. Распакуйте архив в любое удобное место.
3. Запустите `Parser.exe` — откроется окно настроек.
4. В игре нажмите назначенную горячую клавишу (по умолчанию `F8`), затем левой кнопкой мыши выделите название портала на затемнённом экране.
...
*(Здесь можно добавить скриншоты оверлея, редактора и главного окна. Поместите изображения в папку `screenshots` и вставьте ссылки вида `![оверлей](screenshots/overlay.png)`)*
